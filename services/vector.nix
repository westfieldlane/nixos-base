          ...
          source = ''
            ...
            if dataset == "auditd" {
              ...
              if err == null {
                ...
                if ferr == null {
                  nested = to_string(fields.msg) ?? ""
                  if contains(nested, "=") {
                    inner, ierr = parse_key_value(nested, key_value_delimiter: "=", field_delimiter: " ")
                    if ierr == null { fields = merge(fields, inner) }
                  }

                  .audit.fields = fields
                  if fields.res != null {
                    res = to_string(fields.res) ?? ""
